...
data_dir = 'datasets/hand_dataset/'   # data_dir为脚本所在的文件夹
...
def generate_dir(set_name, root_path):   # 往images和labels文件夹下生成相应的文件夹
    ...
    new_images_dir = os.path.join(
        created_images_dir, set_name)   # 将图片从原来的文件夹复制到该文件夹下
    ...
    for matfile in matlab_annotations:
        ...
        content = sio.loadmat(os.path.join(
            annotation_dir, matfile), matlab_compatible=False)
        ...
        with open(os.path.join(new_annotation_dir, filename+".txt"), "w") as hs:
            for box_idx, box in enumerate(boxes.T):
                ...
                hs.write("0 %f %f %f %f" % (norm_center_x,
                                            norm_center_y, norm_width, norm_height))  # 0表示类别
...
if __name__ == '__main__':
    ...
    # 第一个参数表示生成的文件夹的名称
    generate_dir(
        "train", f"{ori_root}hand_dataset/training_dataset/training_data")
    generate_dir("test", f"{ori_root}hand_dataset/test_dataset/test_data")
    generate_dir(
        "validation", f"{ori_root}hand_dataset/validation_dataset/validation_data")
